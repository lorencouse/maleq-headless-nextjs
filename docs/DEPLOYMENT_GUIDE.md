...
| `maleq-news-review.php` | Phone-friendly approval queue for machine-drafted News posts. Serves a standalone mobile page at `/news-review?k=<secret>` (cover, headline, full story per draft) with one-tap **Publish** (fires `maleq-news-autoshare.php` exactly like WP admin), **Delete** (force-deletes the cover attachment but only *trashes* the post so its `_maleq_news_source_url` meta keeps the dedupe guard alive), and **Later** (snoozes to the bottom). Also serves its own service worker + web-app manifest so the page can be installed to the home screen and receive web push (sent by `scripts/news-agent/notify-review.ts` after each cron drafting run; subscriptions stored as JSON in the `maleq_news_review_push_subs` option). Requires the `MALEQ_NEWS_REVIEW_KEY` + `MALEQ_NEWS_REVIEW_VAPID_PUBLIC` constants below. | `wordpress/mu-plugins/maleq-news-review.php` |
