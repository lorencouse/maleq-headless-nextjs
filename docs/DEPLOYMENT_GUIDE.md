...
| `maleq-news-autoshare.php` | Social sharing for the LGBTQ news agent. **(1) Auto-share:** when a news-agent draft is **published** (your approval), it shares the post once to Bluesky + Mastodon — fires only for `post`s in the `news` category carrying the `_maleq_news_source_url` marker, so hand-written guides/products never share. Mirrors `scripts/news-agent/social/*` and reuses the same `_maleq_news_share_urls` / `_maleq_news_shared_at` postmeta, so `sync-shares.ts` stays a compatible manual fallback. **(2) Manual X/Threads:** adds a "Share to X / Threads" meta box on the editor of any News-category post with pre-composed, editable text (the *same* drafted `_maleq_news_social_text` hook + cleaned `_maleq_news_hashtags` + link the Bluesky/Mastodon adapters use, so all four platforms read consistently) and Share-via-intent + Copy buttons — for the two platforms that have no script-free auto-post path. The manual buttons need no extra credentials; Bluesky/Mastodon auto-share still requires the credential constants below. | `wordpress/mu-plugins/maleq-news-autoshare.php` |
| `maleq-news-review.php` | Phone-friendly approval queue for machine-drafted News posts. Serves a standalone mobile page at `/news-review?k=<secret>` (cover, headline, full story per draft) with one-tap **Publish** (approves into a **fixed-slot publish queue** — stories go out only at `MALEQ_NEWS_PUBLISH_SLOTS`, default 9 AM/12/3/6/9 PM `MALEQ_NEWS_PUBLISH_TZ`, five a day, using WordPress's own `future`/`post_date` scheduling; the first `MALEQ_NEWS_FRONT_PICKS_PER_DAY` approvals of a day, default 5, take the earliest slots and later approvals become the long-term backlog, so one sitting can cover several days; autoshare fires on the `future → publish` transition), **Delete** (force-deletes the cover attachment but only *trashes* the post so its `_maleq_news_source_url` meta keeps the dedupe guard alive), and **Later** (snoozes to the bottom). Also serves its own service worker + web-app manifest so the page can be installed to the home screen and receive web push (sent by `scripts/news-agent/notify-review.ts` after each cron drafting run; subscriptions stored as JSON in the `maleq_news_review_push_subs` option). Requires the `MALEQ_NEWS_REVIEW_KEY` + `MALEQ_NEWS_REVIEW_VAPID_PUBLIC` constants below (and optionally `MALEQ_NEWS_PUBLISH_SLOTS` / `MALEQ_NEWS_PUBLISH_TZ` / `MALEQ_NEWS_FRONT_PICKS_PER_DAY`). | `wordpress/mu-plugins/maleq-news-review.php` |
| `maleq-news-cover-picker.php` | "Cover image" meta box on News posts to replace an auto-selected cover: pre-filled Browse links to the image sources (Pexels / TMDB / Wikimedia Commons / Openverse) seeded from the post's cover keywords, a **Re-roll** button (auto-picks a different candidate), and a paste-URL + **Set as cover** that imports the image as the featured image — optionally through the same headline-overlay pipeline as the auto covers (toggle, default on; falls back to the raw image if compositing is unavailable). REST routes `maleq/v1/cover/{reroll,set}` (gated by `edit_post` + REST nonce); execs the pure Bun helpers `scripts/news-agent/{pick-cover,compose-cover}.ts` from the news-agent dir. Requires the news-agent checkout + Bun on the WP host (`NEWS_AGENT_BUN` / `NEWS_AGENT_DIR` to override paths). | `wordpress/mu-plugins/maleq-news-cover-picker.php` |
...
1. **Copy plugin files** to WordPress mu-plugins directory:
   ...
   cp wordpress/mu-plugins/maleq-news-cover-picker.php /path/to/wordpress/wp-content/mu-plugins/maleq-news-cover-picker.php
   cp wordpress/mu-plugins/maleq-news-review.php /path/to/wordpress/wp-content/mu-plugins/maleq-news-review.php
   ```

   > **Cover picker:** `maleq-news-cover-picker.php` execs the Bun helpers `scripts/news-agent/{pick-cover,compose-cover}.ts`, so it needs the news-agent checkout and Bun on the WP host. Defaults assume `/home/maleq-wp/news-agent` and `/home/maleq-wp/.bun/bin/bun`; override with the `NEWS_AGENT_DIR` / `NEWS_AGENT_BUN` env vars if they differ. TMDB poster re-rolls additionally need `TMDB_API_KEY` + `NEWS_AGENT_TMDB_POSTERS=1` in the news-agent `.env` (see Buyer's/News agent docs).
...
// News mobile review page (maleq-news-review.php)
define('MALEQ_NEWS_REVIEW_KEY', 'long-random-hex');            // openssl rand -hex 32 — the ?k= page secret
define('MALEQ_NEWS_REVIEW_VAPID_PUBLIC', 'vapid-public-key');  // pair with NEWS_REVIEW_VAPID_PRIVATE in the news-agent .env
// Publish cadence (all optional — these are the defaults):
// define('MALEQ_NEWS_PUBLISH_SLOTS', '9:00,12:00,15:00,18:00,21:00'); // '' = publish immediately
// define('MALEQ_NEWS_PUBLISH_TZ', 'America/New_York');
// define('MALEQ_NEWS_FRONT_PICKS_PER_DAY', 5); // approvals/day that jump the long-term queue
```
